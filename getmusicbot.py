...
def lambda_handler(event, context):
    ...
    if len(IDs) == 0: # If they are no new mentions
        ...
        # ============= Check all mentions and collect tweet details if criterias are met ============
        ...
        s3 = boto3.client('s3',region_name="eu-west-2", aws_access_key_id=AWS_ACCESS_TOKEN, aws_secret_access_key=AWS_SECRET,config=boto3.session.Config(signature_version='s3v4')) # Create AWS resource
        bucket = "get-music-bucket" # Name of bucket
        ...
        for content in contents:
            ...
            audio_id = content.split("~")[3].split('/')[4] # audio id is what our audio files have been saved as
            ...
            if audio_id in recognized: # If this audio has already been recognized
                pass
            else:
                # create a pre-signed url to access the audio file in s3
                audio_url = s3.generate_presigned_url('get_object',
                                                                Params={'Bucket': bucket,
                                                                        'Key': f'{s3_path_audio}/{audio_id}.mp3'},
                                                                ExpiresIn=120)
                data = {
                    'api_token': AUDIO_KEY, # recognition API token
                    'url': audio_url, # Return apple_music and spotify
                    'return': 'apple_music,spotify',
                }
                result = requests.post('https://api.audd.io/', data=data) # API request
                ...
                try:
                    ...
                    api.update_status(f"@{author} Hey, i'm sorry i couldn't recognize the audio in this video.\n\nTry some other tweet ", in_reply_to_status_id=tweet_id)
                    continue
                ...
                recognized[audio_id] = [result['status'], r_data]
        ...
        message1 = "@{} Hey, i'm sorry i couldn't do this.\n\nThere is either not a song in this video or i just couldn't find it\n\nYou can scold me now - ^ -"
